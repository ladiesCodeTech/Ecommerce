  ...
  Future<List<ProdutosModel>> getAllProdutos() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (var n in json) {
          try {
            // Tratamento do campo price para receber double
            String priceAsString = n['price']?.toString() ?? "0.0";
      ...
      print("getAllProdutos: Erro ${e.toString()}");
      return [];
    }
  }

  // Método GET para obter um único produto pelo ID
  Future<ProdutosModel> getProduto(int id) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        try {
          // Tratamento do campo price para receber double
          String priceAsString = json['price']?.toString() ?? "0.0";
          double price = double.parse(priceAsString);
          json['price'] = price;
        } catch (e) {
          print("Erro ao converter preço para double: $e");
        ...
        return ProdutosModel.fromJson(json);
      } else {
        print("getProduto: StatusCode ${response.statusCode}");
        throw Exception("Erro ao obter produto: ${response.statusCode}");
      }
    } catch (e) {
      print("getProduto: Erro ${e.toString()}");
